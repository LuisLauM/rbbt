...
It is possible in RStudio assigning a keyboard shortcut for **rbbt**
addins. Just go to *Tools –\> Modify Keyboard Shortcuts*, a small window
will show you the available commands. Then, in the **Scope** column,
find the *Addin* type and define the shortcuts for **rbbt** commands.
For example: to set a shortcut for running `bbt_update_bib()`, go to the
row assigned for **“Update bibliography for current document from
Zotero”**, click on the **Shortcut** column and press the desired key
combination.

It should be noted that RStudio comes by default configured with some
keyboard shortcuts, so the overlaping should be avoided (if this
happens, a warning icon will be displayed in the shortcut assignment
window).

Besides, for the commands to work correctly, they must be executed
having the Rmd or Qmd file in the active tab in RStudio. That is to say,
both addins mentioned above will detect the citations and the
bibliography file from the active tab (from the main .Rmd or .Qmd file)
from where the shortcut is executed (i.e. where the cursor is when the
shortcut is called), otherwise you will receive the following error
message: **“Currently selected editor is not a .qmd, .rmd or .Rmd
file”**.
...
- `bbt_detect_citations` now has the abilty for detecting citations
  selectively both in and out chunks. Selectively means that the
  searching will be performed only in header of chunks (i.e. inside of
  `{..., fig.cap = '...', ...`) and the chunk lines that starts with
  `#| x-cap: ''` (where `x` is could be defined with the argument
  `prefix2rm` and by default includes “fig-”, “tbl-”, “eq-”, “sec-”,
  “lst-” and “thm-”).
- Add functionality for citing R packages. Now, it is possible to cite a
  R package in a Rmd/Qmd file using the syntax `@rpkg_NameOfPackage`
  (e.g. `@rpkg_rbbt`). `bbt_detect_citations` and `bbt_update_bib` are
  now prepared for identifying and adding to the bibliography using
  `citation` and `toBibtex` tools internally.
